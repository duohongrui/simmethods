...
CancerInSilico_estimation <- function(ref_data,
  ...
  if(!requireNamespace("CancerInSilico", quietly = TRUE)){
    cat("CancerInSilico is not installed on your device\n")
    cat("Installing CancerInSilico...\n")
  ...
  ##############################################################################
  if(verbose){
    cat("Estimating parameters using CancerInSilico\n")
  }
  # Seed
  set.seed(seed)
  # Estimation
...
#'
#' @param parameters A object generated by \code{\link[CancerInSilico]{inSilicoGeneExpression}}
#' @param other_prior A list with names of certain parameters. Some methods need
#' extra parameters to execute the estimation step, so you must input them. In
#' simulation step, the number of cells, genes, groups, batches, the percent of
#' DEGs and other variables are usually customed, so before simulating a dataset
...
#'
#' @importFrom CancerInSilico inSilicoGeneExpression
...
CancerInSilico_simulation <- function(parameters,
  ...
  cat(glue::glue("nGenes: {data_dim[1]}"), "\n")
  ##############################################################################
  ####                            Simulation                                 ###
  ##############################################################################
  if(verbose){
    cat("Simulating datasets using CancerInSilico\n")
  }
  # Seed
  set.seed(seed)
  # Estimation
  ...
  ##############################################################################
  ## counts
  counts <- simulate_result[["expression"]][, 1:data_dim[2]]
